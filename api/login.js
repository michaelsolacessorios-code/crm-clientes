...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Usuários de Garantia (recebem e conferem a mercadoria devolvida). Ficam numa lista
    // própria (crm:garantia_contas) e não têm usuário nem senha padrão: só existem depois que o
    // gerente cadastra em Credenciais de acesso.
    const contasGarantia = (await getKV(SUPABASE_URL, 'crm:garantia_contas', headers)) || [];
    const contaGarantia = Array.isArray(contasGarantia) ? contasGarantia.find(c => c.usuario === usuario) : null;
    if (contaGarantia) {
      if (contaGarantia.bloqueado) {
